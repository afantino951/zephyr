...
scanning board.yml files under boards/), this script:
  ...
  4. Renames per-board overlay/conf files under tests/**/boards/,
     samples/**/boards/, and boards/shields/**/boards/ that are named
     "<board>_<soc>_(hazard3|m33).{overlay,conf}" to their "_0"
     counterpart - copied instead of renamed for boards that keep their
     bare board files around, renamed (git mv) otherwise.
...
def find_per_board_overlay_conf_ops(board_name, soc, old_cluster, new_cluster):
    ...
    ones under a "socs/" dir) named "<board>_<soc>_<cluster>.{overlay,conf}"
    ...
    the "_0" form."""
    ops = []
    old_stem = f"{board_name}_{soc}_{old_cluster}"
    new_stem = f"{board_name}_{soc}_{new_cluster}"
    mode = "copy" if board_name in COPY_INSTEAD_OF_RENAME else "rename"
...
def find_yaml_qualifier_edits(board_qualifiers):
    ...
    to scope the substitution so we never touch an unrelated board/soc
...
def build_plan(filter_names):
    ...
    plan.yaml_edits = find_yaml_qualifier_edits(
