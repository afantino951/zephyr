...
/* Absolute address where the CPU1 binary is stored in flash. */
...
/*
 * If CPU0 has a zephyr,sram-cpu1-partition chosen property, it implies that
 * CPU1 is configured to run from SRAM. CPU0 must move CPU1's image to SRAM
 * before launching CPU1.
 */
#define HAS_CPU1_SRAM DT_HAS_CHOSEN(zephyr_sram_cpu1_partition)
...
#else
	if (!address_in_range(cpu1_pc, CPU1_CODE_ADDR, CPU1_CODE_SIZE)) {
		LOG_ERR("CPU1 reset pointer 0x%08x invalid.", cpu1_pc);
		return -EINVAL;
	}
#endif /* HAS_CPU1_SRAM */
...
#else
	uint32_t cpu1_image_base = CPU1_CODE_ADDR;
#endif /* HAS_CPU1_SRAM */
